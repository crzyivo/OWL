...
public class CategoriaDAO {
	...
	public void insertarCategoria(CategoriaVO categoria, JDBCTemplate jdbctemp) {
		...
		try {
			...
			String queryString = "INSERT INTO categoria " + "(nombre, nlibros " + "VALUES (?,?)";

			jdbctemp.executeSentence(queryString, categoria.getNombre(), categoria.getNLibros());
	...
	public void annadirLibroaCategoria(CategoriaVO categoria, LibroVO libro, JDBCTemplate jdbctemp) {
		...
		try {
			/* Create "preparedStatement". */
			String queryString = "INSERT INTO pertenece " + "(categoria, titulo, autor " + "VALUES (?,?,?)";

			jdbctemp.executeSentence(queryString, categoria.getNombre(), libro.getTitulo(), libro.getAutor());
	...
	public void actualizarCategoria(CategoriaVO categoria, JDBCTemplate jdbctemp) {
		try {
			/* Create "preparedStatement". */
			String queryString = "UPDATE categoria " + "SET nombre = ?, nlibros = ?  WHERE  nombre = ?)";
			jdbctemp.executeSentence(queryString, categoria.getNombre(), categoria.getNLibros(), categoria.getNombre());
	...
	public List<LibroVO> obtenerLibrosInCategoria(String nombre, JDBCTemplate jdbctemp) {
		...
		try {
			...
			/* Create "preparedStatement". */
			sql = "SELECT  l.titulo, l.autor, descripcion, ventas, id" + "  FROM libro l,pertenece p WHERE "
					+ "p.categoria" + " = " + "'" + nombre + "'" + "and p.titulo=l.titulo and p.autor=l.autor";
	...
	public void borrarCategoria(String nombre, JDBCTemplate jdbctemp) {
		...
		try {
			/* Create "preparedStatement". */
			String queryString = "DELETE * FROM categoria " + "WHERE nombre = " + nombre;
