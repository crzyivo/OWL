...
public class EjemplarDAO {
	...
	public void insertarEjemplar(EjemplarVO ejemplar, JDBCTemplate jdbctemp) {
		...
		try {
			...
			String queryString = "INSERT INTO ejemplar "
					+ "(libro, editorial, estado, precio, vendedor, comprador, ISBN, anno,fechaVenta)"
					+ "VALUES (?,?,?,?,?,?,?,?,?)";

			jdbctemp.executeSentence(queryString, ejemplar.getLibro(), ejemplar.getEditorial(), ejemplar.getEstado(),
					ejemplar.getPrecio(), ejemplar.getVendedor(), ejemplar.getComprador(), ejemplar.getISBN(),
					ejemplar.getAnno(), ejemplar.getfventa());
	...
	public void actualizarEjemplar(EjemplarVO ejemplar, JDBCTemplate jdbctemp) {
		try {
			/* Create "preparedStatement". */
			String queryString = "UPDATE ejemplar " + "SET libro = ?, editorial = ?, estado = ?, "
					+ " precio = ?, vendedor = ?, comprador = ?, ISBN = ?, anno = ?, fechaVenta = ?,fechaCompra = ? WHERE  id = ?";
			jdbctemp.executeSentence(queryString, ejemplar.getLibro(), ejemplar.getEditorial(), ejemplar.getEstado(),
					ejemplar.getPrecio(), ejemplar.getVendedor(), ejemplar.getComprador(), ejemplar.getISBN(),
					ejemplar.getAnno(), ejemplar.getfventa(), ejemplar.getfcompra(), ejemplar.getId());
	...
	public EjemplarVO obtenerEjemplar(int id, JDBCTemplate jdbctemp) {
		...
		try {
			...
			String sql = "SELECT  libro, editorial, estado, precio, vendedor, comprador,"
					+ " ISBN, anno,fechaVenta,fechaCompra, id FROM ejemplar WHERE  id = '" + id + "'";
			...
			if (!resultSet.isEmpty()) {
				...
				String fventa = (String) resultSet.get(8);
				String fcompra = (String) resultSet.get(9);
				int ids = Integer.parseInt((String) resultSet.get(10));

				ejemplarVO = new EjemplarVO(ids, libro, editorial, estado, precio, vendedor, comprador, ISBN, anno,
						fventa, fcompra);
			}
	...
	public List<EjemplarVO> obtenerEjemplaresBy(String parametro, String valueifstring, int valueifint,
		...
		try {
			if (parametro.equals("libro") || parametro.equals("anno")) {
				...
				sql = "SELECT  libro, editorial, estado, precio, vendedor, comprador,"
						+ " ISBN, anno,fechaVenta,fechaCompra, id FROM ejemplar WHERE " + parametro + " = " + "'"
						+ valueifint + "' ORDER BY precio asc";
			} else if (parametro.equals("editorial") || parametro.equals("comprador") || parametro.equals("vendedor")) {
				/* Create "preparedStatement". */
				sql = "SELECT  libro, editorial, estado, precio, vendedor, comprador,"
						+ " ISBN, anno,fechaVenta,fechaCompra, id FROM ejemplar WHERE " + parametro + " = " + "'"
						+ valueifstring + "' ORDER BY precio asc";
			...
			while (!resultSet.isEmpty()) {
				...
				resultSet.remove(0);
				String fventa = (String) resultSet.get(0);
				resultSet.remove(0);
				String fcompra = (String) resultSet.get(0);
				resultSet.remove(0);
	...
	public void borrarEjemplar(int id, JDBCTemplate jdbctemp) {
		...
		try {
			/* Create "preparedStatement". */
			String queryString = "DELETE * FROM ejemplar " + "WHERE id = " + id;
