...
public class ErrorsStringsEjemplares {
    ...
    //private final static List<String> LISTA_CENTIMOS_POSIBLES = Arrays.asList("00,05,09,10,15,19,20,25,29,30,35,39,40,45,49,50,55,59,60,65,69,70,75,79,80,85,89,90,95,99".split(","));
    private final static List<String> LISTA_ESTADOS_CONSERVACION = Arrays.asList("excelente,bueno,deteriorado".split(","));
    ...
    public static void compruebaDatos(EjemplarVO ejemplar, List<String> errores) {
        ...
        } else if (editorial.length() < MIN_LENGTH_EDITORIAL ||
                editorial.length() > MAX_LENGTH_EDITORIAL) {
            errores.add("La editorial debe tener entre " + MIN_LENGTH_EDITORIAL + " y " + MAX_LENGTH_EDITORIAL + " caracteres.");
        }
        ...
        if (ISBN != null &&
                (ISBN.length() < MIN_LENGTH_ISBN || ISBN.length() > MAX_LENGTH_ISBN)) {
             errores.add("Debes especificar un ISBN-10 o un ISBN-13 válido. No deberías escribir guiones ni otros caracteres separadores.");
        }
