...
public class ErrorsStrings {
	...
	private final static Pattern pattern_tlfn = Pattern.compile("^(.*[0-9]){5}.*$");
	...
	public static void compruebaDatos(OwlUserVO usuario, List<String> errores) {
		...
		final Matcher mtlf = pattern_tlfn.matcher(Integer.toString(usuario.getTlfn())); // Cuestionable si tlfn deberia ser integer
		...
		if (usuario.getApellidos() == null || usuario.getApellidos().equals("")) {
			errores.add("Debes introducir tu apellido.");
		...
		if (usuario.getTlfn() == null || !mtlf.matches()) { // tlfn.equals("")) si fuera string
			...
			errores.add(
					// TODO: CORREGIR, el teléfono es un entero
					"El teléfono de contacto debe tener al menos 9 dígitos y ser menor que " + Integer.toString(Integer.MAX_VALUE));
		}
		if (usuario.getCalle() == null || usuario.getCalle().equals("")) {
			...
			errores.add("El nombre de la calle debe tener entre 3 y 120 caracteres.");
		}
		if (usuario.getPoblacion() == null || usuario.getPoblacion().equals("")) {
			errores.add("Debes especificar tu núcleo de población.");
		} else if ((usuario.getCalle()).length() < 3 || usuario.getCalle().length() > 120) {
			errores.add("El nombre de tu núcleo de población debe tener entre 3 y 120 caracteres.");
		}
