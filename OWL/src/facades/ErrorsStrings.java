...
public class ErrorsStrings {
    ...
    public static void compruebaDatos(OwlUserVO usuario, List<String> errores) {
        ...
        } else if (usuario.getEmail().length() < MIN_LENGTH_EMAIL ||
                   usuario.getEmail().length() > MAX_LENGTH_EMAIL) {
        ...
        } else if (usuario.getNacimiento() < MIN_NACIMIENTO ||
                   usuario.getNacimiento() > MAX_NACIMIENTO) {
        ...
        if (usuario.getApellidos() == null || usuario.getApellidos().equals("")) {
            ...
            errores.add("Los apellidos deben tener entre " + MIN_LENGTH_APELLIDOS + " y " + MAX_LENGTH_APELLIDOS + " caracteres.");
        }
        if (usuario.getTlfn() == null) { // tlfn.equals("")) si fuera string
            errores.add("Debes proporcionar un teléfono de contacto.");
        } else if (usuario.getTlfn() < MIN_TELEFONO ||
                   usuario.getTlfn() > MAX_TELEFONO) {// usuario.getTlfn() > 9999999999999999999) esta fuera de rango de integer
        ...
        } else if (usuario.getCalle().length() < MIN_LENGTH_CALLE ||
                   usuario.getCalle().length() > MAX_LENGTH_CALLE) {
            errores.add("El nombre de la calle debe tener entre " + MIN_LENGTH_CALLE + " y " + MAX_LENGTH_CALLE + " caracteres.");
        }
        if (usuario.getPoblacion() == null || usuario.getPoblacion().equals("")) {
            ...
            errores.add("El nombre de tu núcleo de población debe tener entre " + MIN_LENGTH_POBLACION + " y " + MAX_LENGTH_POBLACION + " caracteres.");
        }
        ...
        if (usuario.getPassword() == null || usuario.getPassword().equals("")) {
            ...
            errores.add("La contraseña debe tener entre " + MIN_LENGTH_PASSWORD + " y " + MAX_LENGTH_PASSWORD + " caracteres.");
        }
    }
}
